...
class _InfoPageState extends State<InfoPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: ListView(
        children: [
          ProductImages(task: widget.task),
          TopRoundedContainer(
            color: Colors.white,
            child: Column(
              children: [
    ...
    return Column(
      children: [
        SizedBox(
          ...
          child: AspectRatio(
            aspectRatio: 1,
            child: Image.network('http://10.0.2.2:8000/images/${widget.task.gambar}', errorBuilder: (context, error, stackTrace) => Icon(Icons.broken_image),),
          ),
        ),
        // SizedBox(height: 20),
      ],
            ...
            child: Row(
              children: [
                Text(
                  task.status,
                  style: TextStyle(
                      fontWeight: FontWeight.w600, color: task.status == 'selesai' ? Colors.green : Colors.red),
                ),
                SizedBox(width: 5),
                Icon(
